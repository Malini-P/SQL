...
(2002,'CHK1002','2026-01-17',12000.00),
...
-- 1. Counting how many Productline in Products column

select count(productLine) as Total_ProductLine 
from Products;

-- 2. Sum the total Creditlimit in Customers column

select sum(creditLimit) as Total_CreditLimit
from Customers;

-- 3. Finding the Max & min value stock from Products 

select max(quantityInStock) as Max_Stock ,
min(quantityInStock) as Min_Stock
from Products;

-- 4. Finding average amount in Payments
...
-- 1. Counting MSRP > 5000 from Products

select count(MSRP) as MSRP
from Products
where MSRP > 5000;

-- 2. Sum the total amount where payment is happened on 2nd month

select sum(amount) as Amount
from Payments
where month(paymentDate) = 2;

-- 3. Finding Avg Price for the ordered quantity greater than 1

select avg(priceEach) as Avg_Price
from orderDetails
where quantityOrdered > 1;

-- 4. Finding Max and Min Buyprice if quantitystock is greater than 50

select max(buyPrice) as Max_Amount ,
min(buyPrice) as Min_Amount
from Products
...
-- 1. Counting the Customers From each state

select state,count(state) as State
from Customers
group by(state);
...
-- 3. Average MSRP per Product line
...
-- 1. find customers whose credit limit is above average 
...
-- 1. Customers who paid more than 15000 total
...
-- 2. Product lines equal to 1 product
...
-- 3. Orders where total revenue exceeds 10000
